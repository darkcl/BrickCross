...
#define DEFAULT_GAME_ID "brickGameMultiGame000"
...
void CPhotonLib::initLib(ExitGames::Photon::PhotonListener* PhotonListener, MultiPlayerBrickGameScene* sample)
{
	mpSample = sample;

	//Create PhotonPeer interface
	mpPeer = new ExitGames::Photon::LitePeer(PhotonListener);
	mpPeer->setDebugOutputLevel(DEBUG_LEVEL_INFO);
...
{
	const char* SERVER = "darkcl.no-ip.info:5055";
...
void CPhotonLib::run(void)
{
	...
	case stateLeaving:
		CCLOG("leaving game");
		mpPeer->opLeave(DEFAULT_GAME_ID);
		break;
